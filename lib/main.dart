...
//
// //// ===============================
// //// Project: PropertyBroker (Flutter Web)
// //// Multi-file version with richer UI (hover effects, gradients, hero, chips)
// //// Copy each section into the indicated file path.
// //// No extra packages needed (pure Flutter Material 3).
// //// ===============================
// //
// //
// // // =================================
...
// // import 'pages/broker_profile_page.dart';
// //
// // void main() => runApp(const PropertyApp());
// //
// // class PropertyApp extends StatefulWidget {
// //   const PropertyApp({super.key});
// //   @override
...
// //       themeMode: themeMode,
// //       onGenerateRoute: (settings) {
// //         // Support deep-links like /p/<id> and /b/<id>
...
// //           final prop = mockProperties.firstWhere(
// //                 (p) => p.id == id,
// //             orElse: () => mockProperties.first,
// //           );
// //           return MaterialPageRoute(
// //             builder: (_) => PropertyDetailPage(property: prop, testMode: testMode),
// //           );
// //         }
// //
// //         if (uri.pathSegments.length >= 2 && uri.pathSegments.first == 'b') {
// //           final id = uri.pathSegments[1];
// //           final broker = mockBrokers.firstWhere(
// //                 (b) => b.id == id,
// //             orElse: () => mockBrokers.first,
// //           );
// //           return MaterialPageRoute(
// //             builder: (_) => BrokerProfilePage(broker: broker, testMode: testMode),
// //           );
// //         }
// //
// //         return MaterialPageRoute(builder: (_) => home());
...
// import 'pages/broker_profile_page.dart';
//
// // (Optional) nice clean URLs on web (no #):
// // Add flutter_web_plugins to pubspec and uncomment next 3 lines.
// // import 'package:flutter_web_plugins/flutter_web_plugins.dart';
// // void _usePathUrlStrategy() => setUrlStrategy(const PathUrlStrategy());
//
// void main() {
//   // if (kIsWeb) _usePathUrlStrategy();
//   runApp(const PropertyApp());
// }
//
// class PropertyApp extends StatefulWidget {
...
//   }
//
//   Broker? _findBroker(String id) {
//     for (final b in mockBrokers) {
//       if (b.id == id) return b;
//     }
//     return null;
//   }
...
//         );
//       }
//       return _notFound(settings, 'Property not found');
//     }
//
//     // Broker: /b/:id
//     if (uri.pathSegments.length == 2 && uri.pathSegments.first == 'b') {
//       final id = uri.pathSegments[1];
//       final bro = _findBroker(id);
//       if (bro != null) {
//         return MaterialPageRoute(
//           builder: (_) => BrokerProfilePage(broker: bro, testMode: testMode),
//           settings: settings,
//         );
//       }
//       return _notFound(settings, 'Broker not found');
//     }
//
//     return _notFound(settings, 'Page not found');
//   }
//
//   Route<dynamic> _notFound(RouteSettings s, String msg) {
//     return MaterialPageRoute(
//       builder: (_) => Scaffold(
...
import 'models/property.dart';
import 'models/broker.dart';
import 'models/mock_data.dart'; // your mock lists (2 brokers / 5 properties or full)
import 'pages/home_page.dart';
import 'pages/property_detail_page.dart';
import 'pages/broker_profile_page.dart';

void main() {
  // Clean web URLs (no #/) so /p/:id and /b/:id work nicely when shared
...
class _PropertyAppState extends State<PropertyApp> {
  ...
  Route<dynamic>? _onGenerateRoute(RouteSettings settings) {
    ...
    if (uri.pathSegments.length == 2 && uri.pathSegments.first == 'p') {
      final id = uri.pathSegments[1];
      final prop = _findPropertyById(id);
      if (prop != null) {
        return MaterialPageRoute(
          settings: settings,
          builder: (_) => PropertyDetailPage(property: prop, testMode: testMode),
    ...
    if (uri.pathSegments.length == 2 && uri.pathSegments.first == 'b') {
      final id = uri.pathSegments[1];
      final broker = _findBrokerById(id);
      if (broker != null) {
        return MaterialPageRoute(
          settings: settings,
          builder: (_) => BrokerProfilePage(broker: broker, testMode: testMode),
  ...
  Broker? _findBrokerById(String id) {
    for (final b in mockBrokers) {
      if (b.id == id) return b;
    }
    return null;
