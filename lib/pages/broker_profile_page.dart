...
class BrokerProfilePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final props = mockProperties.where((p) => p.broker.id == broker.id).toList();
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ListTile(
            leading: CircleAvatar(backgroundImage: NetworkImage(broker.photoUrl), radius: 26),
            title: Text(broker.name, style: const TextStyle(fontWeight: FontWeight.w700)),
            subtitle: Column(
              ...
              children: [
                ...
                Text('${broker.dealsClosed} deals • ${broker.email} • ${broker.phone}'),
              ],
            ),
          ),
          const SizedBox(height: 12),
            ...
            Card(
              child: ListTile(
                leading: ClipRRect(
                  ...
                  child: Image.network(p.imageUrl, width: 64, height: 48, fit: BoxFit.cover),
                ),
                ...
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => PropertyDetailPage(property: p, testMode: testMode)),
                ),
              ),
            ),
        ],
