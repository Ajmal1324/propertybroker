...
//   });
//
//   /// Build a canonical URL to this property:
//   /// - On web, use current origin so it works on any domain (preview, custom).
//   /// - On mobile, fall back to Firebase Hosting domain.
//   String _propertyUrl(String id) {
//     if (kIsWeb) {
//       return Uri.base.replace(path: '/p/$id', query: '').toString();
//     }
//     return 'https://propertybroker-test.web.app/p/$id';
//   }
//
//   @override
//   Widget build(BuildContext context) {
...
//     final deepLink = _propertyUrl(p.id);
//
//     return Scaffold(
//       appBar: AppBar(
...
//                 ),
//                 const SizedBox(width: 8),
//                 // Inline share/copy (in addition to AppBar) on wide layouts
//                 Wrap(
//                   spacing: 6,
//                   children: [
//                     IconButton(
...
//                       tooltip: 'Share',
//                       icon: const Icon(Icons.share),
//                       onPressed: () => Share.share('Check this property: ${widget.deepLink}'),
//                     ),
//                   ],
//                 ),
//               ],
...
class PropertyDetailPage extends StatelessWidget {
  ...
  String _propertyUrl(String id) {
    // Web: use current origin (works on any host)
    if (kIsWeb) return Uri.base.replace(path: '/p/$id', query: '').toString();
    // Mobile: your hosting domain
    return 'https://propertybroker-test.web.app/p/$id';
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          ListTile(
            ...
            leading: CircleAvatar(backgroundImage: NetworkImage(p.broker.photoUrl)),
            title: Text(p.title, style: const TextStyle(fontWeight: FontWeight.w800)),
            subtitle: Text('${p.address}, ${p.city}', maxLines: 1, overflow: TextOverflow.ellipsis),
            trailing: TextButton.icon(
              onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => BrokerProfilePage(broker: p.broker, testMode: testMode)),
              ),
              icon: const Icon(Icons.open_in_new),
          ...
          const SizedBox(height: 16),
          const Text('Amenities', style: TextStyle(fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
...
class _BottomBar extends StatelessWidget {
  const _BottomBar({required this.priceStr, required this.onCall, required this.onMeet, required this.onShare});
  final String priceStr;
  final VoidCallback onCall;
  final VoidCallback onMeet;
  final VoidCallback onShare;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(color: Theme.of(context).colorScheme.surface, boxShadow: [
        BoxShadow(color: Colors.black.withOpacity(0.06), offset: const Offset(0, -2), blurRadius: 7),
      ]),
      padding: const EdgeInsets.fromLTRB(16, 10, 16, 10),
      child: SafeArea(
        top: false,
        child: Row(
          children: [
            ...
            const SizedBox(width: 6),
            OutlinedButton.icon(onPressed: onMeet, icon: const Icon(Icons.event_available), label: const Text('Meeting')),
            const SizedBox(width: 6),
            ElevatedButton.icon(onPressed: onCall, icon: const Icon(Icons.phone), label: const Text('Contact')),
          ],
        ),
      ),
    );
